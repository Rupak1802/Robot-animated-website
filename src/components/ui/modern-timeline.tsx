...
import { CheckCircle, Clock, Circle, type LucideIcon } from "lucide-react"
  ...
  return (
    ...
    >
      <div className="relative">
        <div 
          className="absolute left-4 sm:left-6 top-0 bottom-0 w-px bg-zinc-200 dark:bg-zinc-800" 
          aria-hidden="true"
        />
        
        <motion.div
          className="absolute left-4 sm:left-6 top-0 w-px bg-black dark:bg-white origin-top"
          initial={{ scaleY: 0 }}
          whileInView={{ 
            scaleY: 1,
            transition: {
        ...
        <div className="space-y-8 sm:space-y-12 relative">
          {items.map((item, index) => {
            ...
            const IconComponent = item.icon || getStatusIcon(item.status)
              ...
              >
                <div className="flex items-start gap-4 sm:gap-6 text-left">
                  <div className="relative flex-shrink-0">
                    ...
                    >
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full overflow-hidden border-2 border-background shadow-lg relative z-10 flex items-center justify-center bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
                        {item.image ? (
                          <img
                            src={item.image}
                      ...
                      <CardContent className="p-4 sm:p-6">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-3">
                          <div className="flex-1 min-w-0">
                            <motion.h3 
                              className="text-lg font-semibold text-foreground mb-1 group-hover:text-black dark:group-hover:text-white transition-colors duration-300"
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                              ...
                              {item.category && item.date && (
                                <span className="w-1 h-1 bg-zinc-300 dark:bg-zinc-700 rounded-full" aria-hidden="true" />
                              )}
                              {item.date && (
                          ...
                          <Badge 
                            className={cn(
                              "w-fit text-[10px] font-medium border rounded-full px-2 py-0.5",
                              config.badgeBg,
                              config.badgeText,
                              "border-current/20"
                            )}
                        ...
                        <motion.p 
                          className="text-sm text-zinc-650 dark:text-zinc-400 leading-relaxed mb-4"
                          initial={{ opacity: 0.8 }}
        ...
        <motion.div
          ...
          aria-hidden="true"
        >
          <div className="w-3 h-3 bg-black dark:bg-white rounded-full shadow-sm" />
        </motion.div>
      </div>
    </section>
  )
